...
public class TenantController extends IdevelopController {
	...
	@PreAuth("hasPerm('system:common:all')")
	public R<IPage<Tenant>> list(@ApiIgnore @RequestParam Map<String, Object> tenant, Query query, IdevelopUser idevelopUser) {
		QueryWrapper<Tenant> queryWrapper = Condition.getQueryWrapper(tenant, Tenant.class);
		IPage<Tenant> pages = tenantService.page(Condition.getPage(query), (!idevelopUser.getTenantId().equals(IdevelopConstant.ADMIN_TENANT_ID)) ? queryWrapper.lambda().eq(Tenant::getTenantId, idevelopUser.getTenantId()) : queryWrapper);
		return R.data(pages);
	}
	...
	public R<List<Tenant>> select(Tenant tenant, IdevelopUser idevelopUser) {
		QueryWrapper<Tenant> queryWrapper = Condition.getQueryWrapper(tenant);
		List<Tenant> list = tenantService.list((!idevelopUser.getTenantId().equals(IdevelopConstant.ADMIN_TENANT_ID)) ? queryWrapper.lambda().eq(Tenant::getTenantId, idevelopUser.getTenantId()) : queryWrapper);
		return R.data(list);
	}
